    ...
    if ($file_size > 20 * 1024 * 1024) {
        ...
        try {
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if (count($data) < 5) {
                    $error_count++;
                    $error_details[] = "Row $row: Insufficient columns (expected 5, got " . count($data) . ")";
                    continue;
                }
                ...
                $position   = isset($data[3]) ? trim($data[3]) : '';
                $type       = isset($data[4]) ? strtolower(trim($data[4])) : 'delegate';
                
                // Validate required fields
                if (empty($first_name) || empty($last_name)) {
                    $error_count++;
                    $error_details[] = "Row $row: Missing first or last name";
                    continue;
                }
                ...
                $qr_code = "ATTENDEE|" . uniqid() . "|" . $first_name . "|" . $last_name . "|" . $company . "|" . $position;

                // Prepare batch data
                $batch_data[] = [
                    $first_name, $last_name, $company, $position, $type, $qr_code, $user_id
                ];

                // Insert in batches
                if (count($batch_data) >= $batch_size) {
                    insertBatch($batch_data, $conn);
            ...
            if ($success_count > 0) {
                ...
                if ($error_count > 0) {
                    $message .= '<div class="alert alert-warning">⚠️ ' . $error_count . ' rows failed.</div>';
                }
            } else {
...
function insertBatch($batch_data, $conn) {
    ...
    $sql = "INSERT INTO attendees (first_name, last_name, company, position, type, qr_code, registered_by) 
                    ...
                    <div class="format-card p-4 mb-4">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <code class="text-white bg-transparent">first_name,last_name,company,position,type</code>
                            </div>
                            <div class="col-md-6">
                                <h6 class="mb-2">Example:</h6>
                                <code class="text-white bg-transparent">John,Doe,Acme Inc,Manager,delegate</code>
                            </div>
                        </div>
                        <div class="mt-3">
